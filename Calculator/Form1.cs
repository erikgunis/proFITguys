...
         }         public void setThings()
        ...
        }            private void textBox1_TextChanged(object sender, EventArgs e)         {             if ((Globals.operand == '\0'))             {                  if (!(String.IsNullOrEmpty(textBox1.Text)))                 {                     Globals.baseNumber = float.Parse(textBox1.Text);                 }             }             if (!(Globals.operand == '\0'))             {                 if (!(string.IsNullOrEmpty(textBox1.Text)) && (!Globals.enterPressed))                 {                     Globals.secondNumber = float.Parse(textBox1.Text);
                    Globals.intNumber = Globals.secondNumber;                 }             }         }          private void Form1_Press(object sender, KeyPressEventArgs e)         {             if (!(ActiveControl.GetType() == typeof(TextBox)))             {                 textBox1.Text += e.KeyChar;                 textBox1.SelectionStart = textBox1.Text.Length;                 textBox1.Focus();             }         }          private void Form1_KeyDown(object sender, KeyEventArgs e)         {//https://msdn.microsoft.com/en-us/library/aa243025(v=vs.60).aspx

            if (Globals.operandset && (e.KeyValue >= 48 && e.KeyValue <= 57))             {                //essageBox.Show("oko");                 Globals.potentialNegative = true;                 Globals.operandset = false;                 textBox1.Clear();             }                                      int key = e.KeyValue;             if ( key == 67 )
            {
                setDefaults();
                textBox1.Clear();
...
                 //Globals.labelFirstNumber = false;                 Globals.potentialNegative = false;             
